...
void *train_logic(void *train_number)
{
    ...
    while (num_station_visits < 10)
    {
        ...
        *is_T_waiting_to_enter_station = false;
        ...
        sleep_ms(3000 + clock() % 21 * 100 * (1 + (my_number == 1) * 1));
...
void *logVariables(void *Thread_Number)
{
    ...
    int sample_index = 0;
    std::ofstream outFile("/home/ani/Documents/Runtime_Verification/CS632_2025_assignment2/Tooled_SUV/output.txt"); // Create an output file stream
    ...
    if (outFile.is_open())
    {
        ...
        outFile << std::setw(6) << "S_idx"
                << std::setw(5) << "T0_r"
                << std::setw(5) << "T0_w"
                << std::setw(5) << "T0_s"
                << std::setw(5) << "T1_r"
                << std::setw(5) << "T1_w"
                << std::setw(5) << "T1_s" << std::endl;
        while (logging_active)
        {
            sleep_ms(50);

            outFile << std::setw(6) << sample_index
                    << std::setw(5) << is_T0_running
                    << std::setw(5) << is_T0_waiting_to_enter_station
                    << std::setw(5) << is_T0_in_station
                    << std::setw(5) << is_T1_running
                    << std::setw(5) << is_T1_waiting_to_enter_station
                    << std::setw(5) << is_T1_in_station
                    << std::endl;
            sample_index++;
        }
